#!/bin/bash
#
# OpenClaw Installation Script for macOS 11 Big Sur
...
echo -e "${BLUE}================================${NC}"
echo -e "${BLUE}OpenClaw Installer for macOS 11 Big Sur${NC}"
echo -e "${BLUE}================================${NC}"
echo ""
...
if [ "$ARCH" = "arm64" ]; then
    echo -e "${GREEN}Apple Silicon (M1/M2) detected${NC}"
    echo -e "${GREEN}Big Sur has full Apple Silicon support${NC}"
fi
...
# Check Rosetta 2 for Apple Silicon
if [ "$ARCH" = "arm64" ]; then
    if ! /usr/bin/pgrep oahd &>/dev/null; then
        warning "Rosetta 2 not installed. Installing..."
        /usr/sbin/softwareupdate --install-rosetta --agree-to-license
    fi
    success "Rosetta 2 available"
fi
...
# For Apple Silicon, ensure Rust is properly configured
if [ "$ARCH" = "arm64" ]; then
...
echo ""
echo -e "For Apple Silicon Macs:"
echo -e "  - Native ARM64 binary installed"
echo -e "  - Rosetta 2 available for x86 compatibility"
echo ""
